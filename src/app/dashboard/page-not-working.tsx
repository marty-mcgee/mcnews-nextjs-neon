...
import { RefreshCw, Filter, X, Car, Radio, AlertTriangle, Calendar, MapPin, Download, Globe, Activity, Layers, Eye, EyeOff, ChevronLeft, ChevronRight, Info } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [layers, setLayers] = useState([
    { id: 'caltrans', name: 'Caltrans', icon: <Car className="w-4 h-4" />, color: 'blue', enabled: true, count: 0 },
    { id: 'bayarea511', name: '511.org', icon: <Radio className="w-4 h-4" />, color: 'emerald', enabled: true, count: 0 },
    { id: 'chp-live', name: 'CHP Live', icon: <AlertTriangle className="w-4 h-4" />, color: 'red', enabled: true, count: 0 },
    { id: 'chp-historical', name: 'Historical', icon: <Calendar className="w-4 h-4" />, color: 'purple', enabled: false, count: 0 },
  ]);

  const fetchAllData = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/dashboard?showAll=${showAllRegions}&historical=${showHistorical}`
      );
      ...
      const { caltrans, bayArea511, chpLive, chpHistorical } = result.data;
      const events: MapEvent[] = [];
      let counts = { caltrans: 0, bayarea511: 0, 'chp-live': 0, 'chp-historical': 0 };
      
      (caltrans || []).forEach((item: any) => {
        if (item.latitude && item.longitude) {
          events.push({
            id: `caltrans_${item.closure_id || item.source_id || item.id}`,
            source: 'caltrans',
      ...
      if (showHistorical) {
        (chpHistorical || []).forEach((item: any) => {
          if (item.latitude && item.longitude) {
            events.push({
              id: `chphist_${item.id}`,
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
        <Card className="text-center">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">{currentPageWithCoords.length} on map</p>
          </CardContent>
        </Card>
        ...
        {layers.map((layer) => (
          <button
            ...
            className={`rounded-xl p-4 text-center transition-all duration-200 hover:shadow-md ${
              layer.enabled 
                ? `bg-${layer.color}-50 dark:bg-${layer.color}-950/30 border-2 border-${layer.color}-400 dark:border-${layer.color}-500` 
                : 'bg-muted/30 border border-border hover:bg-muted/50'
            }`}
          >
            <div className="flex items-center justify-center gap-2 mb-1">
              <span className={layer.enabled ? `text-${layer.color}-600 dark:text-${layer.color}-400` : 'text-muted-foreground'}>
                {layer.icon}
              </span>
              <p className={`text-2xl font-bold ${layer.enabled ? `text-${layer.color}-600 dark:text-${layer.color}-400` : 'text-muted-foreground'}`}>
                {layer.count}
              </p>
            </div>
            <p className={`text-sm font-medium ${layer.enabled ? `text-${layer.color}-600 dark:text-${layer.color}-400` : 'text-muted-foreground'}`}>
              {layer.name}
            </p>
            <div className="flex items-center justify-center gap-1 mt-1">
              {layer.enabled ? <Eye className="w-3 h-3 text-green-500" /> : <EyeOff className="w-3 h-3 text-muted-foreground" />}
              <span className="text-xs text-muted-foreground">{layer.enabled ? 'Visible' : 'Hidden'}</span>
            </div>
          </button>
        ))}
      </div>
        ...
        <CardContent className="p-0 overflow-hidden rounded-xl">
          {mapEvents.length > 0 ? (
            ...
            <div className="h-[600px] bg-muted flex flex-col items-center justify-center">
              ...
              <p className="text-muted-foreground">No events with location data on this page</p>
              <p className="text-sm text-muted-foreground mt-1">Try adjusting filters or toggling data layers</p>
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <table className="w-full">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs uppercase">Severity</th>
              </tr>
            </thead>
                  ...
                  <tr className="hover:bg-muted/50 cursor-pointer" onClick={() => toggleRowExpansion(event.id)}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${getSourceBadge(event.source)}`}>
                        {event.source === 'caltrans' ? 'Caltrans' : event.source === 'bayarea511' ? '511.org' : event.source === 'chp-live' ? 'CHP Live' : 'CHP Historical'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm">{event.type}</td>
                    <td className="px-4 py-3 text-sm">{event.location?.substring(0, 60)}</td>
